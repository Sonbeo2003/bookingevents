...
function AddServiceCenter({ closeForm, onServiceCenterAdded }) {
  ...
  useEffect(() => {
    const loadServicesAndCenters = async () => {
      try {
        // Fetch services
        const servicesResponse = await fetch(`${url}/API_Events/getdichvu.php`);
        const servicesData = await servicesResponse.json();
        setServices(servicesData);

        // Fetch centers
        const centersResponse = await fetch(`${url}/API_Events/gettrungtam.php`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${url}/API_Events/themdichvutrungtam.php`, {
        method: "POST",
        headers: {
